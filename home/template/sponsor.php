<section class="page-section bg-white" id="sponsor">
    <div class="container ">
        <div class="text-center">
            <h2 class="section-heading text-uppercase">Sponsor</h2>
        </div>
        <div class="row">
            <div class="col-lg-3">
                <div class="team-member1">
                    <img class="mx-auto rounded-circle" src="image/sponsor/elmech.png" alt="..." />
                    <h6>CV. ELMECH TECHNOLOGY INDONESIA</h6>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member1">
                    <img class="mx-auto rounded-circle" src="image/sponsor/elprint.png" alt="..." />
                    <br>
                    <h6>ELPRINT</h6>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member1">
                    <img class="mx-auto rounded-circle" src="image/sponsor/elkost.png" alt="..." />
                    <h6>ELKOST</h6>

                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member1">
                    <img class="mx-auto rounded-circle" src="image/sponsor/academy.png" alt="..." />
                    <h6>Automation Academy</h6>

                </div>
            </div>
            
                <div class="col-lg-3">
                    <div class="team-member1">
                        <img class="mx-auto rounded-circle" src="image/sponsor/ap.png" alt="..." />
                        <h6>CV. ASSEMBLYPRO PERKASA</h6>

                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="team-member1">
                        <img class="mx-auto rounded-circle" src="image/sponsor/mokko.jpg" alt="..." />
                        <h6>PT. MOKKO INDONESIA</h6>

                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="team-member1">
                        <img class="mx-auto rounded-circle" src="image/sponsor/bccustom.png" alt="..." />
                        <h6>BC CUSTOM</h6>
                    </div>
                </div>

            </div>

        </div>
    </div>
</section>
<!-- Team-->
<!-- <section class="page-section" style="background-color :#e9ecef;" id="team">
    <div class="container">
        <div class="text-center">
            <h2 class="section-heading text-uppercase">Media Partner</h2>
        </div>
        <div class="row">
            <div class="col-lg-3">
                <div class="team-member">
                    <img class="mx-auto rounded-circle" src="image/infolomba.jpg" alt="..." />
                    <h6>Info.lomba</h6>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member">
                    <img class="mx-auto rounded-circle" src="image/evenkampus.png" alt="..." />
                    <h6>Info Campus</h6>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member">
                    <img class="mx-auto rounded-circle" src="image/apaaja.png" alt="..." />
                    <h6>Event Apaja</h6>

                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member">
                    <img class="mx-auto rounded-circle" src="image/infoeven.png" alt="..." />
                    <h6>Info Event</h6>

                </div>
            </div>
        </div>

        <div class="row">
            <div class="col-lg-3">
                <div class="team-member">
                    <img class="mx-auto rounded-circle" src="image/logolomba.png" alt="..." />
                    <h6>Info Lomba</h6>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member">
                    <img class="mx-auto rounded-circle" src="image/infokti.png" alt="..." />
                    <h6>Info Lomba KTI</h6>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member">
                    <img class="mx-auto rounded-circle" src="image/lombaonline.jpg" alt="..." />
                    <h6>Lomba Online</h6>

                </div>
            </div>
            <div class="col-lg-3">
                <div class="team-member">
                    <img class="mx-auto rounded-circle" src="image/circle.png" alt="..." />
                    <h6>Info Lomba & Beasiswa</h6>

                </div>
            </div>
        </div>
    </div>
</section> -->
<!-- Contact-->
<!-- <section class="page-section" style="background-color: '#E9ECF;'"> -->
