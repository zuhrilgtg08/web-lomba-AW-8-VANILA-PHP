...
$excel->setActiveSheetIndex(0)->setCellValue('B1', "NAMA TIM"); 
$excel->setActiveSheetIndex(0)->setCellValue('C1', "NAMA KETUA"); 
$excel->setActiveSheetIndex(0)->setCellValue('D1', "NAMA ANGGOTA 1");
$excel->setActiveSheetIndex(0)->setCellValue('E1', "NAMA ANGGOTA 1");
$excel->setActiveSheetIndex(0)->setCellValue('F1', "ASAL SEKOLAH");
$excel->setActiveSheetIndex(0)->setCellValue('G1', "No HP");
$excel->setActiveSheetIndex(0)->setCellValue('H1', "Jenis Lomba"); 
...
$lomba = $_GET['lomba'];
$sql = $kon->kueri("SELECT b.*, lp.`id_peserta`, lp.`nama_tim`, lp.`nama`, lp.`nama_anggota1`, lp.`nama_anggota2`, lp.`sekolah`, lp.`email`, lp.`telp`, lp.`pass`, lp.`jenis_lomba`, lp.`bidanglomba`, lp.`judul_abstrak`, lp.`proses`
FROM `tb_bukti` AS b
LEFT JOIN `login_peserta` AS lp
ON b.`id_peserta` = lp.`id_peserta`
WHERE lp.`id_peserta` > 1 AND jenis_lomba = '$lomba' AND proses > 1
...
foreach ($sql as $m) {
	...
	$excel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, $m['nama_tim']);
	$excel->setActiveSheetIndex(0)->setCellValue('C'.$numrow, $m['nama']);
	$excel->setActiveSheetIndex(0)->setCellValue('D'.$numrow, $m['nama_anggota1']);
	$excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, $m['nama_anggota2']);
	$excel->setActiveSheetIndex(0)->setCellValue('F'.$numrow, $m['sekolah']);
	$excel->setActiveSheetIndex(0)->setCellValue('G'.$numrow, $m['telp']);
	$excel->setActiveSheetIndex(0)->setCellValue('H'.$numrow, $m['jenis_lomba']);
	...
	$excel->getActiveSheet()->getColumnDimension('H')->setWidth(25); // Set width kolom category G
...
// Set judul file excel page setup
	...
	$excel->getActiveSheet(0)->setTitle("excelPeserta");
	$excel->setActiveSheetIndex(0);
// end judul

// Proses file excel
	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	header('Content-Disposition: attachment; filename="excelPeserta.xlsx"'); // Set nama file excel nya
